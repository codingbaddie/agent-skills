...
3.  **陌生讀者檢核 (Cold Reader Check)**（2026-07 失聯自動化 PRD 定稿時歸納）：假設讀者沒參與過任何討論，逐段自問「他讀得懂嗎」。
    -   **專有名詞首次出現，要麼自帶定義、要麼給出路**（一句話解釋或連結到定義處）。❌ 「供原因標籤取值」 -> ✅ 「供『原因』欄取值（首週消失／連續失聯／反覆失聯，定義見 §7.3）」。
...
5.  **事件驅動架構 (Event-Driven Architecture)**：對於複雜邏輯，採用「事件驅動」格式描述行為，協助 BE/QA 拆解狀態機。格式如下：
    -   **[時間/場合] 事件：[主體 + 動作]**
    -   **系統判斷**：[當下邏輯判定條件]
    -   **資料狀態 (BE)**：[API/DB 的狀態變更]
    -   **UI / Metric 狀態 (FE)**：[前端渲染、樣式與指標的具體變化]
6.  生成或更新 `_context/PRD.md` (或子路徑)。
7.  **限制**：在使用者核准規格書之前，不要寫程式碼。
